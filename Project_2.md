...
<ul>
 ...
 The activation signal (), further helps in defining a transfer function F(~e, ) which later defines the respective output vector ~u of the system i.e. F(~e, ) = ~u. The data from sensors, (e.g. distance measurements, acceleration) also forms an input vector <b> <i> e &#8712; Rm </i> </b>. The activation  also limits the activity a of the behaviour B, such that   a. Behaviour B can thus be described as:
